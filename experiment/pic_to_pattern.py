...
if len(sorted_name) == 0:
    ...
    # 数组前面填充8次
    sorted_name = [sorted_name[0]] * 8 + sorted_name
    print(time.strftime("Start the clock,%Y-%m-%d %H:%M:%S", time.localtime()))
    ...
    print(time.strftime("End the clock, %Y-%m-%d %H:%M:%S", time.localtime()))
